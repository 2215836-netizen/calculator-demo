...
### 9.1 단위 테스트 (자동화 - Browser TDD)
- **대상**: `src/core/` (계산 로직, 파서, 알고리즘), `src/utils/` (포맷터)
- **도구**: 브라우저 기반 커스텀 테스트 러너 (`test.html`)
- **방식**: Node.js가 없는 환경 특성상 브라우저 콘솔을 통한 결과 검증
- **규칙**: UI 요소(DOM)에 대한 의존성 없이 순수 로직만 테스트

### 9.2 UI/통합 테스트 (수동)
- **대상**: `src/ui/`, `index.html`, 이벤트 핸들러, DOM 조작
- **도구**: 없음 (사람이 직접 수행)
- **이유**: UI 자동화 테스트의 유지보수 비용 절감 및 빠른 이터레이션
- **절차**:
  1. 기능 구현 후 브라우저에서 직접 클릭/입력
  2. 주요 시나리오(사칙연산, 공학함수, 예외처리) 수행
  3. 디자인 및 반응형 동작 육안 확인
